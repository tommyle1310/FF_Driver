...
const FFSwipe: React.FC<FFSwipeProps> = ({ onSwipe, direction }) => {
  ...
  const [swipedDirection, setSwipedDirection] = useState<'left' | 'right' | null>(null); // Track the swipe direction
  ...
  const onGestureEvent = (event: any) => {
    ...
    if (direction === 'left') {
      ...
      translateX.setValue(Math.max(translationX, -(screenWidth - SWIPER_WIDTH))); // Limit to right edge
    }
  };
  ...
  const onHandlerStateChange = (event: any) => {
    ...
    if (state === 5) { // 5 indicates that the gesture has ended
      ...
      if (Math.abs(translationX) >= (screenWidth - (SWIPER_WIDTH + 50))) {
        ...
        Animated.spring(translateX, {
          toValue: direction === 'left' ? -(screenWidth - SWIPER_WIDTH ) : screenWidth - SWIPER_WIDTH - 24,
  ...
  return (
    ...
    >
      ...
      >
        <IconFeather name='chevrons-right' size={32} color={swipeEdgeReached ? '#111' :'#4caf50'} />
      </Animated.View>
    </PanGestureHandler>
  );
};
