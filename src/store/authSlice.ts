...
export const loadTokenFromAsyncStorage = createAsyncThunk('auth/loadToken', async () => {
  ...
  const available_for_work = await AsyncStorage.getItem('available_for_work') === 'true';  // ensure it's converted to a boolean
  ...
  console.log('AsyncStorage data on load:', { available_for_work });
  return {
    accessToken,
...
export const saveTokenToAsyncStorage = createAsyncThunk(
  ...
  async (data: {
    ...
    await AsyncStorage.setItem('available_for_work', JSON.stringify(data.available_for_work));
    ...
    await AsyncStorage.setItem('contact_email', JSON.stringify(data.contact_email));
    console.log('Saved contact_email:', data.contact_email);

    await AsyncStorage.setItem('contact_phone', JSON.stringify(data.contact_phone));
...
const authSlice = createSlice({
  ...
  reducers: {
    setAuthState: (state, action) => {
      const { accessToken, app_preferences, balance, email, fWalletId, driverId, userId, user_type, contact_email, contact_phone, available_for_work } = action.payload;
  ...
  extraReducers: (builder) => {
    builder
      .addCase(loadTokenFromAsyncStorage.fulfilled, (state, action) => {
        const { accessToken, app_preferences, balance, fWalletId, email, userId,  driverId,user_type, contact_email, contact_phone, available_for_work } = action.payload;
      ...
      .addCase(saveTokenToAsyncStorage.fulfilled, (state, action) => {
        const { accessToken, app_preferences, balance, fWalletId, email, userId, user_type, driverId,contact_email, contact_phone, available_for_work } = action.payload;
        state.accessToken = accessToken;
        state.isAuthenticated = true;
        state.available_for_work = available_for_work;
        state.app_preferences = app_preferences;
